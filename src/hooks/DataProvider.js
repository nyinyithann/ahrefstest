...
function loadCountries(dispatch, signal) {
  var callback = function (result) {
    ...
    try {
      var cjson = JSON.parse(
        '{ "countries" : ' + gist._0.files.countryJsonFile.content + ' }'
      );
  ...
  WebAPI.getCountries(
    'https://api.github.com/gists/659db3f4566df459bd59c8a53dc9f71f',
    callback,
    Caml_option.some(signal),
    undefined
  );
}
...
function useCountriesData(param) {
  ...
  return [state.loading, state.error, state.countries];
}

export { initialState, reducer, loadCountries, useCountriesData };
